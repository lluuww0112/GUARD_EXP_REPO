...
def _select_topk_from_eligible_patches(
    ...
    *,
    ...
    initial_allocated_budget: torch.Tensor,
    initial_keep_budget: torch.Tensor,
    final_allocated_budget: torch.Tensor,
    reallocated_budget: torch.Tensor,
    ...
    keep_counts = final_allocated_budget.to(dtype=torch.long)
    ...
    initial_allocated_list = initial_allocated_budget.tolist()
    initial_keep_list = initial_keep_budget.tolist()
    final_keep_list = keep_counts.tolist()
    reallocated_list = reallocated_budget.tolist()
    ...
    frame_metadata = [
        {
            ...
            "initial_allocated_budget": int(initial_allocated_list[frame_idx]),
            "initial_keep_count": int(initial_keep_list[frame_idx]),
            "final_keep_count": int(final_keep_list[frame_idx]),
            "reallocated_count": int(reallocated_list[frame_idx]),
...
def maskclip_patch_selection(
    ...
    **_: Any,
    ...
    if patch_score_threshold is None:
        raise ValueError(
            "`patch_score_threshold` must be a non-null float for selection_v3."
        )
    ...
    initial_capacities = torch.full(
        image_frame_scores.shape,
        fill_value=tokens_per_frame,
        device=image_frame_scores.device,
        dtype=torch.long,
    )
    initial_allocated_budget, raw_budget, frame_weights = (
    ...
    initial_keep_budget = torch.minimum(initial_allocated_budget, eligible_counts)
    remaining_capacity = eligible_counts - initial_keep_budget
    leftover_budget = total_budget_value - int(initial_keep_budget.sum().item())

    reallocated_budget = torch.zeros_like(initial_keep_budget)
    if leftover_budget > 0 and int(remaining_capacity.sum().item()) > 0:
        reallocated_budget, _, _ = _allocate_budget_with_softmax_capacities(
            image_frame_scores,
            total_budget=leftover_budget,
            capacities=remaining_capacity,
            temperature=temperature,
        )

    final_allocated_budget = initial_keep_budget + reallocated_budget
    selected_indices, frame_metadata = _select_topk_from_eligible_patches(
        ...
        initial_allocated_budget=initial_allocated_budget,
        initial_keep_budget=initial_keep_budget,
        final_allocated_budget=final_allocated_budget,
        reallocated_budget=reallocated_budget,
    ...
    metadata = {
        "selector_type": "maskclip_patch_selection_v3",
        ...
        "reallocated_token_count": int(reallocated_budget.sum().item()),
        ...
        "initial_frame_allocated_budgets": [
            int(budget) for budget in initial_allocated_budget.tolist()
        ],
        "final_frame_allocated_budgets": [
            int(budget) for budget in final_allocated_budget.tolist()
        ],
